...
This portfolio ships a WebGPU-first fluid renderer with the existing WebGL2 ping-pong FBO renderer as the fallback for browsers without `navigator.gpu` or for devices that fail WebGPU initialization.

## Goals

- Run the primary simulation through WGSL compute shaders.
...
5. Regenerate the typography texture on a side loop with a hard timeout so DOM rasterization never blocks the render loop.
...
- `src/components/fluid/webgpu/WebGPUFluidCanvas.tsx`: production WebGPU runtime, feature detection, compute/render pipelines, DOM typography upload, and fallback boundary.
- `src/components/fluid/FluidCanvas.tsx`: current WebGL2 fallback.

## R3F Notes

React Three Fiber remains the WebGL2 fallback path through `FluidCanvas` and `FluidSimulation`. The primary WebGPU path is isolated behind `WebGPUFluidCanvas` because the low-level compute pipeline needs direct control over storage textures, uniform buffers, and command encoding.
